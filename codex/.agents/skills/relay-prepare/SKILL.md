---
name: relay-prepare
description: Prepare a human-in-the-loop relay packet — render the current conversation as a handoff for a collaborating AI agent and save it to a file. Does NOT touch the clipboard; run $relay-copy when you are ready to paste. Invoke only when the user explicitly runs $relay-prepare; never auto-trigger.
---

<!-- Keep behavior in sync with the Claude relay-prepare skill counterpart at claude/.claude/skills/relay-prepare/SKILL.md -->
...
specific target when invoking (e.g. `$relay-prepare claude`), use that name instead.
...
## 3. Compute the target path (one shell call)
...
mkdir -p "$RELAY_DIR"
date +"$RELAY_DIR/%Y%m%d-%H%M%S-codex.md"
```
...
Use your file-editing tool to write the rendered packet to that exact printed
path. Do NOT write it with `echo`/heredoc — the structured write avoids quoting
breakage on multi-line / shell-hostile content.
